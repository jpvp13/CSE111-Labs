...
def Q1(_conn):
    ...
    try:
        sql = """SELECT w_warehousekey, w_name, w_capacity, w_suppkey, w_nationkey FROM warehouse ORDER BY w_warehousekey"""
        args = []

        cur = _conn.cursor()
        cur.execute(sql, args)

        k = '{:>10s} {:<1s} {:>45s} {:>10s} {:>10s}\n'.format("wId","wName","wCap","sId","nId")
        print(k)
        ...
        for row in rows:
            l = '{:>20} {:<35} {:>15d} {:>10d} {:>10d}'.format(row[0], row[1], row[2], row[3], row[4])
            print(l)
...
def Q2(_conn):
    ...
    try:
        sql = """SELECT n_name, COUNT(DISTINCT w_warehousekey), SUM(w_capacity)
        ...
        for row in rows:
            l = '{:>20} {:>50} {:10d}'.format(row[0], row[1], row[2])
            print(l)
...
def Q3(_conn):
    ...
    try:
        ...
        with open ('input/3.in','r') as f:
            next = 1
            for count in f: #checks how may line there are in input file
                if next == 1:   #if there is only 1 line break
                    first = count
                    break
                next = 1 + next #more than 1 line, iterate next to search next line
        first = first.rstrip("\n")  #remove new line
        new = '%'
        new += first
        ...
        sql = """SELECT s_name, n_name, w_name
                ...
                WHERE w_suppkey = s_suppkey
                    ...
                    AND w_name LIKE ?
                 """    # '?' is where argument will be placed
                 
        args = [new]   #this is used as input conditions
        # args = []
        cur = _conn.cursor()
        cur.execute(sql, args)
        ...
        for row in rows:
            l = '{:>0} {:<16} {:>10}'.format(row[0], row[1], row[2])
            print(l)
...
# Q4 finds the warehouses from a given region that have capacity larger than a given threshold.  
# The region name and the minimum capacity are parameters stored in the file input/4.in. Q4 prints 
# the warehouse name and its capacity in decreasing order of the capacity.
def Q4(_conn):
    print("++++++++++++++++++++++++++++++++++")
    print("Q4")
    ...
    try:
        # text = ''
        # num = 0
        # lines = []      #lines[0] holds 'ASIA' and lines[1] holds "2000"
        ...
        with open ('input/4.in','r') as f:
            next = 1
            for count in f: #checks how may line there are in input file
                if next == 2:   #if there is only 1 line break
                    lines2 = count
                    break
                next = 1 + next #more than 1 line, iterate next to search next line
        lines1 = lines1.rstrip("\n")  #remove new line  (this holds ASIA)
        lines2 = lines2.rstrip("\n")  #remove new line  (this holds 2000)
...
# nation in everyregion.  The suppliers’ nation is a parameter stored in the file input/5.in.  
# If there are no warehouses in  a  region,  then  value 0 is  printed  for  that  region. Q5 prints  
# the  region  and  the  capacity  sorted alphabetically by region.
def Q5(_conn):
    print("++++++++++++++++++++++++++++++++++")
    print("Q5")
